...
## 현재 상태 (2026-09-04) — 상세는 `docs/PROGRESS.md`
...
| product-page-gen (#10) | **착수(2026-07-26)** — 큐텐재팬 K뷰티 역직구용 상세페이지 생성 원자. 약기법/화장품법 표현 lint(block=렌더 거부), Qxpress 물류 게이트, 마진 시뮬, 톤 3종 HTML 렌더. 오케스트레이션은 `.claude/skills/product-page`(`/product-page 나이아신 화장품`). 이미지 입력은 사용권 있는 실사/그 기반 AI 생성(aiLabeled 필수)만 — 타사 이미지 재사용 금지(금지선 #1). 테스트 69. **`market: naver-smartstore`+`locale: ko` 한국어 상세 지원(2026-09-06 append, 핸드크림 파일럿)** — 물류·마진 게이트는 큐텐 전용 |
...
| meta-paid-reach (#14) | **착수(2026-09-04, PoC)** — Meta 공식 Marketing API로 테스트 광고 **최소 1,000회 유료 노출** PoC. Campaign→Ad Set→Video→Creative→Ad 생성 경로, 소재 입력 3종(local-file/hosted-url/meta-video-id). 기본 dry-run, 실제 생성도 **전량 PAUSED**(`--execute --confirm CREATE_PAUSED` + 사람 승인 기록 필수). 라이브 집행은 `run --execute --confirm LIVE_SPEND` + 환경변수 이중잠금(`META_POC_ALLOW_LIVE_SPEND=I_UNDERSTAND`·`META_POC_HARD_SPEND_CAP_MINOR`) + Ad Set lifetime_budget/end_time 하드 상한 + 활성화 직전 원격 재검증. impressions/spend/status 폴링 → 1,000회 또는 지출 상한에서 **Campaign부터 PAUSE**. 계약 MetaPaidReachConfig/Snapshot/State append(paid-reach.ts). 테스트 18·타입체크 통과. **실계정 호출·과금 0회** — 다음 = README 「검증 순서」 2~4(preflight→CREATE_PAUSED→Ads Manager 사람 검수), Meta 자산이 없으면 `docs/SETUP-GUIDE.md`(초보자용, 계정→토큰→.env→preflight) 먼저. 브랜치 `feat/meta-paid-reach-poc`(develop 병합). Graph API 기본 `v26.0`은 TODO(D1) 재확인 |
