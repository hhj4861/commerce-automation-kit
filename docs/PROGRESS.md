...
| 2026-07-29 (클라우드 전환) | **apps/shopshorts-cloud 가동** — 대시보드가 Cloudflare Pages(고정 무료 도메인 shopshorts-dash.pages.dev)+D1(큐 단일 진실 소스)+R2(영상)로 이전. 실행은 로컬 워커(apps/shopshorts/worker.mjs) 하이브리드: lint 검증(승인 직후, 위반 시 자동 반려)·finalize(Yooni TTS+동기 조립→R2 업로드)·핫키워드 push(30분)·하트비트(UI 워커 온라인 배지). 사람 게이트는 Functions 전이 화이트리스트+lintChecked/최종영상/자리표시자 링크 서버 강제. **배포 전 적대적 리뷰 워크플로(26 에이전트)로 확정 결함 22건(치명 3: 발행 데드엔드·반려잡 과금 조립·lint 게이트 우회) 수정 후 배포**. 클립 생성(힉스필드)·대본 작성은 Claude 세션 모니터가 클라우드 큐 폴링. 구 로컬 서버(server.mjs)는 폐기 예정(워커로 대체). 터널 방식은 폐기(회사망 QUIC 차단·URL 유동) |
