...
export async function startStudioService({env=process.env,call=accountBroker(env),start=startStudioWorker,log=console.error}={}) {
  ...
  const worker=start({env:{...withoutCredentials(env),...values,CAK_CLOUD_SECRETS_ACTIVE:'1'},cloud,token:values.SHOPSHORTS_TOKEN,workDir:env.SHOPSHORTS_WORK_DIR || resolve(import.meta.dirname,'data/work'),keepAlive:true});
