...
export function executionStatus({project,step,config,connectionUnknown=false}, now=Date.now()) {
  ...
  if(task?.state==='done' && steps[task.action]===step)return {kind:'done',title:`${name}을 완료했어요`,detail:scenario?`${project.scenes.length}개 장면을 확인하고 다듬어 주세요.`:'결과를 확인한 뒤 다음 단계로 진행하세요.'};
