...
export function createRecommendationFocus({ input, anchor, summary, onCancel, onApply, onRetry }) {
  const kind = input.focus === 'topic' ? '이야기' : '분위기';
  const started = Date.now();
  let pending = true, disposed = false, result = null, currentStage = null, currentProvider = null;
  const dialog = document.createElement('dialog');
  dialog.className = 'recommend-dialog';
  dialog.setAttribute('aria-labelledby', `recommend-title-${input.focus}`);
  dialog.innerHTML = `<header class="recommend-dialog-head"><div><p class="recommend-context">${escape(input.category)} <span>${input.format === 'short' ? '숏폼' : '롱폼'} / ${input.duration}초</span><span data-provider></span></p><h2 id="recommend-title-${input.focus}" tabindex="-1">${kind} 3가지를 제안할게요</h2></div><button class="recommend-dismiss" type="button" aria-label="추천 취소하고 닫기">×</button></header>
    ...
    <footer class="recommend-dialog-foot"><span data-footer-note>추천 후 하나를 골라 기획에 적용하세요.</span><button class="secondary recommend-cancel" type="button">취소하고 닫기</button></footer>`;
  ...
  const dismiss = () => { if (pending) onCancel(); close(); };
  ...
  function update({ stage, provider }) {
    ...
    summary.textContent = `${kind} 추천 ${stage === 'authorization' ? '연결 대기' : '진행 중'}${provider ? ` · ${provider === 'claude' ? 'Claude' : 'Codex'}` : ''}`;
  ...
  function complete(data) {
    ...
    dialog.querySelector('[data-footer-note]').textContent = `${data.provider === 'claude' ? 'Claude' : 'Codex'} · ${elapsed()}초 소요 · 적용 전까지 입력 내용은 유지됩니다.`;
    ...
    reopen.textContent = `${kind} 추천 ${data.suggestions.length}개 다시 보기`; reopen.onclick = open; summary.append(reopen);
    open(); content.scrollTop = 0; title.focus();
  }
  function fail(error) {
    if (disposed) return;
    finishControls();
  ...
  return { update, complete, fail, destroy() { disposed = true; stop(); if (dialog.open) dialog.close(); dialog.remove(); } };
}
