...
- **사용자가 컨펌한 시안(스토리보드)은 산출 계약이다.** 컷 구성·타이밍·오버레이 문구·폰트 스타일·크기·색상 모두 시안과 동일하게 산출해야 하며, 생성·후반 단계에서 임의로 바꾸지 않는다. 시안과 다르게 나오면 결함으로 취급하고 시안에 맞춰 재작업한다.
- **오버레이는 drawtext 가 아니라 "시안 CSS → 투명 PNG → ffmpeg overlay 합성"으로 만든다** — drawtext 는 자간·라운드 박스를 지원하지 않아 시안과 달라진다. 방법: 1920x1080 투명 HTML(시안과 동일 CSS) → `Chrome --headless=new --default-background-color=00000000 --screenshot` → `overlay=0:0:enable='between(t,a,b)'` (페이드는 `format=rgba,fade=t=in:st=..:d=..:alpha=1`). 시안을 고치면 같은 CSS 로 PNG 만 다시 뽑으면 된다(비용 0).
- 오버레이 기준 크기(1920x1080): 라벨 56px(박스 포함 시각 72px급)·리뷰 인용 54px·출처 32px·브랜드 150px(자간 .32em)·태그 44px — 폰·쇼케이스 카드 가독 기준.
...
## 포커스 연출 극대화 (2026-08-10 — 사용자 확정 규칙)

- **광고에서 소구점(포커스) 비트의 "비주얼 연출"은 과장이 기본값이다.** 문제 비트는 시청자가 즉각 아프게 느낄 극한 강도로(예: 난반사 → 베일링 글레어로 도로가 안 보일 정도), 해소 비트는 거의 초현실적인 대비로, 제품 비트는 소재 질감 숭배 수준으로 미화한다.
- 자동화: 컨셉 beats 에 `emphasis: 'problem'|'resolution'|'hero'` 를 지정하면 build-prompt 가 극대화 지시문을 자동 주입한다. 포커스에 해당하는 비트에는 emphasis 지정을 기본으로 한다.
- **경계(법적): 과장은 연출(비주얼)에만 허용된다.** 문구·수치·기능의 사실 주장(자막·내레이션·설명란)은 과장 금지 — "반사가 줄어든다"는 되지만 "100% 차단"은 실증 없이는 불가(표시광고법). 리뷰 인용은 원문 그대로.
