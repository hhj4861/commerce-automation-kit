 ...
 * 인자 조립은 전부 core/ffargs.ts(순수)에서 하고, 여기는 실행·타임아웃·에러 투명화만 담당.
 *
 * 실패 원인 투명화: 비정상 종료 시 stderr 마지막 줄을 에러 메시지에 포함한다.
 * ffmpeg 미설치(ENOENT)는 설치 안내를 담은 명확한 에러로 변환한다.
 */
import { spawn } from 'node:child_process';
import { z } from 'zod';
import { buildProbeArgs } from '../core/ffargs.js';
...
/** 입력 영상의 해상도·길이 조회 (ffprobe JSON 파싱 + zod 검증) */
